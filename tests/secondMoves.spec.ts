...
test('2番目のワザ取得', async ({ page }) => {
  for (const [name, url] of secondMoves) {
    ...
    const titleHandle = await page.locator('//h2[text()="ワザ"]/following-sibling::h4[2]').evaluateHandle(renderStroke, theme.title);
    const descHandle = await page.locator('//h2[text()="ワザ"]/following-sibling::p[2]').evaluateHandle(renderStroke, theme.desc);
