...
const theme: Theme = {
  ...
  subtitle: {
    ...
    border: 'thick ridge rgb(50, 50, 50, 0.8)',
    padding: '3px 5px',
    width: '400px',
  },
  title: {
...
test('特殊エネルギー取得', async ({ page }) => {
  for (const [name, url] of energies) {
    ...
    const titleHandle = await page.locator('#title').evaluateHandle(renderStroke, theme.title);
    const descHandle = await page.locator('//h2[text()="特殊エネルギー"]/following-sibling::p[1]').evaluateHandle(renderStroke, theme.desc);
